...
PUBLIC_NETWORK_PASSPHRASE = stellar_sdk.Network.PUBLIC_NETWORK_PASSPHRASE
TESTNET_NETWORK_PASSPHRASE = stellar_sdk.Network.TESTNET_NETWORK_PASSPHRASE

NETWORKS_CONSTS = {
    "MAINNET" : {"name": "MAINNET", "network_passphrase": PUBLIC_NETWORK_PASSPHRASE, "horizon": "https://horizon-testnet.stellar.org"},
    "TESTNET": {"name": "TESTNET", "network_passphrase": TESTNET_NETWORK_PASSPHRASE, "horizon": "https://horizon.stellar.org"},
}
...
class Stellar:
    def __init__(self, name, network, secret=None):
        self.name = name
        ...
        self.secret = secret
        self.update_address_from_secret(secret)

    def update_address_from_secret(self, secret):
        self.address = stellar_sdk.Keypair.from_secret(secret).public_key

    def to_dict(self):
        return {"network": self.network["name"], "name": self.name, "address": self.address, "secret": self.secret}

    def _get_horizon_server(self):
        server_url = self.network["horizon"]
    ...
    def set_unlock_transaction(self, unlock_transaction):
        ...
        """
        txe = stellar_sdk.TransactionEnvelope.from_xdr(unlock_transaction, self.network["network_passphrase"])
        tx_hash = txe.hash()
    ...
    def _unlock_account(self, escrow_account):
        ...
        for unlockhash in escrow_account.unlockhashes:
            ...
            if unlockhash_transation is None:
                return
            print(unlockhash_transation["transaction_xdr"])
    ...
    def _merge_account(self, address):
        ...
        transaction_builder = stellar_sdk.TransactionBuilder(
            source_account=account, network_passphrase=self.network["network_passphrase"], base_fee=base_fee
        )
    ...
    def activate_through_friendbot(self):
        ...
        resp.raise_for_status()
        print(f"account with address {self.address} activated and funded through friendbot")
    ...
    def add_known_trustline(self, asset_code):
        ...
        if not issuer:
            raise ValueError(f"There is no known issuer for {asset_code} on network {self.network}")
    ...
    def _change_trustline(self, asset_code, issuer, limit=None, secret=None):
        ...
        try:
            server.submit_transaction(transaction)
            if limit is None:
                print(f"Added trustline {asset_code}:{issuer} to account {self.address}")
        except stellar_sdk.exceptions.BadRequestError as e:
            print(e)
            raise e
    ...
    def transfer(
        ...
        """
        issuer = None
        print(f"Sending {amount} {asset} to {destination_address}")
        if asset != "XLM":
        ...
        transaction = stellar_sdk.TransactionEnvelope.from_xdr(transaction, self.network["network_passphrase"])
    ...
    def _transfer_locked_tokens(
        ...
        if asset_code != "XLM":
            print("Adding trustline to escrow account")
    ...
    def _set_account_signers(self, address, public_key_signer, preauth_tx_hash, signer_kp):
        ...
        print(response)
        print(f"Set the signers of {address} to {public_key_signer} and {preauth_tx_hash}")
    ...
    def modify_signing_requirements(
        ...
        try:
            response = server.submit_transaction(tx)
            print(response)
            print(f"Set the signers of {self.address} to require {signature_count} signers")
        except stellar_sdk.exceptions.BadRequestError:
            print("Transaction need additional signatures in order to send")
            return tx.to_xdr()
    ...
    def sign(self, tx_xdr: str, submit: bool = True):
        ...
        source_keypair = stellar_sdk.Keypair.from_secret(self.secret)
        tx = stellar_sdk.TransactionEnvelope.from_xdr(tx_xdr, self.network["network_passphrase"])
        tx.sign(source_keypair)
        if submit:
            horizon_server = self._get_horizon_server()
    ...
    def sign_multisig_transaction(self, tx_xdr):
        ...
        try:
            self.sign(tx_xdr)
            print("Multisig tx signed and sent")
        except UnAuthorized as e:
            print("Transaction needs additional signatures in order to send")
    ...
    def _manage_sell_order(
        ...
        """
        server = self._get_horizon_server()
        tb = TransactionBuilder(self.load_account(), network_passphrase=self.network["network_passphrase"])
        try:
            tx = (
